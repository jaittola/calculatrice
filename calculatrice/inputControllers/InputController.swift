...
class InputController: ObservableObject {
    ...
    private func subscribeToInputValues() {
        stringValueCancellable = activeInputBuffer.$stringValue.sink { [weak self] newStringValue in
            guard let self else { return }
            self.stringValue = newStringValue
            self.value = Value(NumericalValue(activeInputBuffer.doubleValue, originalStringValue: newStringValue))
        }
    }
}
